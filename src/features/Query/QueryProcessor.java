...
public class QueryProcessor {
    ...
    public QueryProcessor(PositionalIndex index) {
        ...
        arTokenizer  = new features.Text_Preprocessing.Arabic.Tokenizer();
        arNormalizer = new features.Text_Preprocessing.Arabic.Normalizer();
        arStopWords  = new features.Text_Preprocessing.Arabic.StopWordsRemover();
        arStemmer    = new features.Text_Preprocessing.Arabic.Stemmer();
    }
    ...
    private List<String> preprocessEnglish(String query) {
        List<String> tokens = enTokenizer.tokenize(query);
        tokens = enStopWords.removeStopWords(tokens);
        tokens = enStemmer.stem(tokens);
        return tokens;
    }

    private List<String> preprocessArabic(String query) {
        String normalized      = arNormalizer.process(query);
        List<String> tokens    = arTokenizer.tokenize(normalized);
        tokens                 = arStopWords.process(tokens);
        List<String> stemmed   = new ArrayList<>();
        for (String t : tokens) stemmed.add(arStemmer.process(t));
        return stemmed;
    }
    ...
    public List<SearchResult> singleWordQuery(String query) {
        ...
        List<String> terms = preprocess(query);
...
return results;
    ...
    public List<SearchResult> multiWordQuery(String query) {
        ...
        Set<Integer> commonDocs = new HashSet<>(getDocSet(terms.get(0)));

      
        for (int i = 1; i < terms.size(); i++) {
            commonDocs.retainAll(getDocSet(terms.get(i)));
            if (commonDocs.isEmpty()) return Collections.emptyList();
        }

        List<SearchResult> results = new ArrayList<>();
        for (int docId : commonDocs) {
            results.add(new SearchResult(docId, index.getDocName(docId), 1.0));
        }
        return results;
    }
